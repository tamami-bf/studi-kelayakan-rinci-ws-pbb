...
\begin{enumerate}
  ...
  \textit{VPN} merupakan singkatan dari \textit{Virtual Private Network}, yaitu sebuah koneksi privat melalui jaringan publik (dalam hal ini internet). Dengan pengertian yang lebih luas adalah pada 2 (dua) kata, yaitu :
  
  \begin{itemize}
    \item \textbf{virtual network}, dalam arti jaringan yang terbentuk bersifat virtual. Tidak ada koneksi jaringan secara nyata antara 2 (dua) titik yang berhubungan.
    
    \item textbf{private}, yaitu jaringan yang terbentuk bersifat \textit{private} dimana tidak semua orang bisa mengaksesnya. Data yang dikirimkan terenkripsi sehingga tetap rahasia meskipun melalui jaringan publik.
  \end{itemize}
  
  Dengan VPN seolah-olah membuat jaringan di dalam jaringan atau biasa disebut \textit{tunnel}. \textit{Tunneling} adalah suatu cara membuat jalur \textit{private} dengan menggunakan infrastruktur pihak lain. 
  
  Beberapa perangkat \textit{Router} yang sekarang tersedia, biasanya menawarkan implementasi VPN Server sekaligus, sehingga menjadikan biaya pengadaan yang dikeluarkan semakin rendah.
